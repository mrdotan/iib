...
public class DesEncryptor implements Encryptor {
    ...
    public String decryptPassword(byte[] key, byte[] pinData)
        ...
        if (pinData.length != PIN_DATA_SIZE) {
            throw new Exception("the PIN_DATA_SIZE should be " + PIN_DATA_SIZE);
        }
        if (key.length != WORKING_KEY_SIZE) {
            throw new Exception("the key's size should be " + WORKING_KEY_SIZE);
        }
        String k = new String(MessageUtil.byte2hex(key), 0, 32);
        String pin = new String(MessageUtil.byte2hex(pinData));
        System.out.println("use key: " + k);
        //logger.debug("use key:{}", k);
        pin = Des.des3(pin, k, Des.Mode.DECRYPTION);
        System.out.println("after triple des decryption pin : " + pin);
        ...
        System.out.println("after triple des decryption pin : " + pwd);
        //logger.debug("password :{}", pwd);
        return Des.HEX_2_ASC(pwd);
    }

    public byte[] encryptPassword(byte[] key, String password) throws Exception {
        if (password.length() > 8) {
            throw new Exception("the password should be less than 8!");
        }
    ...
    public byte[] decryptWorkingKey(byte[] key, byte[] workingKey) throws Exception {
        if (workingKey.length < WORKING_KEY_SIZE || key.length != WORKING_KEY_SIZE) {
            throw new Exception("the key's size should be 24 bytes!");
        }
        try {
            String k = new String(MessageUtil.byte2hex(key), 0, 32);
           ...
           System.out.print("after decrypt working key: "+ new String(MessageUtil.byte2hex(wkbs)));
           System.out.print("check value:" + new String(MessageUtil.byte2hex(this.checkvalue(wkbs))));
            return wkbs;
        } catch (Exception e) {
    ...
    public byte[] encryptWorkingKey(byte[] key, byte[] workingKey) throws Exception {
    ...
    public byte[] checkvalue(byte[] workingKey) {
        ...
        System.out.println("check value: " +cv);
    ...
    public byte[] encrypt(byte[] key, byte[] data) {
        ...
        System.out.println("des encrypt in : " +d);
        //logger.debug("des encrypt in : {}", d);
        d = Des.des(d, k, Des.Mode.ENCRYPTION);
        System.out.println("des encrypt out: " +d);
    ...
    /**
     * 计算报文认证码 报文认证算法，不一定是8583所独有； 所以它不应该放在转码器中，而是应该存在于加密机中
     *
